...
CREATE OR REPLACE FUNCTION administrative.transfer_lease_data(varchar,varchar)
  RETURNS TABLE(service varchar, gender varchar, kind varchar, amount float, stamp_duty float ,transfer_duty float, registration_fee float) AS
$BODY$
BEGIN
	...
	select holders.display_value service, gender_code gender, type_code, amount::float, stamp_duty::float, transfer_duty::float, registration_fee::float
