-- Function: application.get_work_summary(date, date, character varying)

-- DROP FUNCTION application.get_work_summary(date, date, character varying);

CREATE OR REPLACE FUNCTION application.get_work_summary(IN from_date date, IN to_date date, IN character varying)
  RETURNS TABLE(req_type character varying, req_cat character varying, group_idx integer, in_progress_from integer, on_requisition_from integer, lodged integer, requisitioned integer, registered integer, cancelled integer, withdrawn integer, in_progress_to integer, on_requisition_to integer, overdue integer, overdue_apps text, requisition_apps text) AS
$BODY$
DECLARE 
   tmp_date DATE; 
BEGIN
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION application.get_work_summary(date, date, character varying)
  OWNER TO postgres;
COMMENT ON FUNCTION application.get_work_summary(date, date, character varying) IS 'Returns a summary of the services processed for a specified reporting period';
